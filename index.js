...
function parseArgs(algorithm) {
    program
        ...
        .option('--output-dir <value>', 'Output dir relative to file. Default: \'\' (same dir)', parseInt)
        .parse(process.argv);
}
...
async function compress(algorithm) {
    ...
    if (algorithm === 'brotli') {
        const options = {
            mode: program.brotliMode || 1,
            quality: program.brotliQuality || 11,
            lgwin: program.brotliLgwin || 22,
        ...
        const options = {
            numiterations: program.zopfliNumiterations || 15,
